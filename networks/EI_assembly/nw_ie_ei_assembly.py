...
def main_loop(seed=30001, target_e=3, target_i=10):
    ...
    # Connections
    wII = 3*br.nS        #fixed weigth for II (unless plastic)
    wE = 0.3*br.nS       #fixed weigth for EE and EI
    ...
    br.run(300*second)
    print('Here we introduce an EI assembly and continue running')
    synEE.w['i<784 and j<784'] = 1.5*br.nS 
    synEI.w['i<784 and j<196'] = 1.5*br.nS 
    br.run(100*second)
    br.device.build()

    # Take a screenshot of synIE and synII
    ...
    np.savez(
        os.path.join(br.device.project_dir, "final_weights_and_connectivity_ie_400sec.npz"),
    ...
    print("Saved final weights + connectivity to final_weights_and_connectivity_ie_400sec.npz")
...
def single_run(seed=42, target_e=3, target_i=10):
    ...
    br.device.reinit()
    br.device.activate()
    folder = f"./output/no_autapse_{seed}_{target_e}_{target_i}_5min"
