...
def main_loop(seed=30001, target_e=3, target_i=10, stim=28, folder='output', sim_time=600):
    ...
    save_times_s = [140, 150, 290, 300, 440, 450, 590, 600, 700] # in sec
    ...
    partial_cue = int(stim*7)
    SynP = br.Synapses(P, SI[:partial_cue], 'w : siemens', on_pre='gI += wII')
    SynP.connect(p=0.05)
    SynP.w = 3*nS
    ...
    new_sim_time = sim_time+300

    np.savez(
        os.path.join(br.device.project_dir, f"{new_sim_time}_weights_and_connectivity.npz"),
    ...
    print(f"Saved final weights + connectivity to {new_sim_time}_weights_and_connectivity.npz")
    print('Elapsed time', time.time() - start_time)

def single_run(seed=42, target_e=3, target_i=10, stim=28, sim_time=600):
    """ Function called in main, runs single simulation with given params.
        ...
        Args:
            ...
            stim (int): Stimulation factor that multiply with 7 neurons to decide on the size of the partial cue.
    ...
    br.device.activate()
    new_sim_time=sim_time+300
    folder = f"./output/{seed}_{target_e}_{target_i}_{stim}_{new_sim_time}sec"
    br.set_device('cpp_standalone', build_on_run=False, clean=True)
    # br.prefs.devices.cpp_standalone.openmp_threads = 1
    main_loop(seed, target_e, target_i, stim, folder, sim_time)
    return
...
def main():
    ...
    parser.add_argument("--stim", type=int, required=True, help="Stimulation factor for pattern completion")
    parser.add_argument("--sim_time", type=float, required=True, help="Simulation_time")
    ...
    single_run(seed=args.seed, target_e=args.target_e, target_i=args.target_i, stim=args.stim, sim_time=args.sim_time)
...
#example call: python nw_iall_recall_stim_disinh.py --seed 42 --target_e 3 --target_i 10 --stim 28 --sim_time 600