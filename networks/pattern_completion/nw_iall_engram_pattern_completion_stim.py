...
def main_loop(seed=30001, target_e=3, target_i=10, stim=28, folder='output'):
    ...
    save_times_s = [150, 290, 300, 450, 590, 600, 700] # in sec
    ...
    SynP = br.Synapses(P, SE[:partial_cue], 'w : siemens', on_pre='gE += w')
    SynP.connect(p=0.05)
    SynP.w = 3*nS
    ...
    br.run(300*second)
    print('Here we would introduce an engram and continue running')
...
def single_run(seed=42, target_e=3, target_i=10, stim=28):
    ...
    br.device.reinit()
    br.device.activate()
    folder = f"./output/engram_{seed}_{target_e}_{target_i}_{stim}_615sec"
    br.set_device('cpp_standalone', build_on_run=False, clean=True)
    # br.prefs.devices.cpp_standalone.openmp_threads = 1
    main_loop(seed, target_e, target_i, stim, folder)
    return
...
def main():
    ...
    single_run(seed=args.seed, target_e=args.target_e, target_i=args.target_i, stim=args.stim)
